...
\begin{proof}
Assume by means of contradiction that $\equiv$ is simple but that there is a $\equiv$-poset~$\preccurlyeq^\equiv_\sigma$ whose Hasse diagram contains a cycle~$C$.
We draw this Hasse diagram on the table~$(i, \sigma^{-1}_i)$ of the permutation~$\sigma$.
Let~$j$ be the topmost node of~$C$.
Hence~$j$ has two incoming edges~$i \to j$ and~$k \to j$, one arising from an ascent of~$\sigma$, and one arising from a descent of~$\sigma$, so that~$i < j < k$.
Assume for instance that~$i$ appears below~$k$, meaning $\sigma^{-1}_i < \sigma^{-1}_k$ (the other case is symmetric).
Consider the leftmost point~$\ell$ of the table of~$\sigma$ in rectangle~${]i, k]} \times {]\sigma^{-1}_i < \sigma^{-1}_k]}$.
Note that~$j < \ell \le k$ as~$(i,j) \in \boxslash(\sigma)$, and that~$(i,\ell) \in \boxslash(\sigma)$.
As~$(i, \ell) \prec (i,j)$ and~$(i,j) \in \boxslash_\equiv(\sigma)$ we therefore obtain that~$\arcUp(i, \ell, \sigma) \notin \arcs_\equiv$.
Let~$\alpha \eqdef (u, v, U, V)$ be a maximal (in the arc poset) arc of~$\arcs_n \ssm \arcs_\equiv$ which is a subarc of~$\arcUp(i, \ell, \sigma)$.
As~$\arcUp(i, j, \sigma)$ and~$\arcUp(j, k, \sigma)$ both belong to~$\arcs_\equiv$, we have~$i \le u < j < v \le \ell$.
As~$\equiv$ is a simple congruence, we have either~$U = \varnothing$ or~$V = \varnothing$.
If~$V = \varnothing$, then~$U = {]u,v[} \ni j$, contradicting that~$\alpha$ is a subarc of~$\arcUp(i, \ell, \sigma)$.
Hence, $U = \varnothing$ and~$V = {]u,v[}$, so that all points of~$V$ appear above~$j$.
Consider now an edge~$p - q$ of~$C$ with~$p < j < q$ (there is at least one such edge to connect~$i$ to~$k$).
As~$j$ is the top vertex of~$C$, the edge~$p-q$ passes below~$j$, hence below all points of~$V$.
This implies that~$\alpha = (u, v, \varnothing, {]u,v[})$ is a subarc of~$\arcUp(p, q, \sigma)$ and~$\arcDown(p, q, \sigma)$.
Hence,~$p-q$ cannot be an edge of~$\preccurlyeq^\equiv_\sigma$, a contradiction.
\end{proof}
