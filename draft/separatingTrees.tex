...
\vincent{add a figure with examples of Schröder separating trees.}
...
\begin{proof}
Consider the contraction~$T$ of an edge~$J_\circ \to J_\bullet$ in a Schröder separating tree~$S$, and denote by~$J_{\circ\bullet} \eqdef J_\circ \sqcup J_\bullet$ the merged node.
...
As~$Y \subset Z$, the subtrees~$X$ and~$Y$ of~$T$ are separated by~$j \in J$.
...
Let~$T$ be the tree obtained by splitting the node~$J_{\circ\bullet}$ of~$S$ into two nodes~$J_\circ$ and~$J_\bullet$, adding an edge~$J_\circ \to J_\bullet$, and attaching each ancestor (resp.~descendant) subtree~$X$ of~$J_{\circ\bullet}$ to~$J_\circ$ if~$\max(X) < \max(J_\circ)$ (resp.~if~$\min(X) < \min(J_\bullet)$) and to~$J_\bullet$ otherwise.
...
\vincent{todo: I need to redo this proof here with the new splitting rule...}
Let~$T$ be the tree obtained by splitting the node~$J_{\circ\bullet}$ of~$S$ into an edge~$J_\circ \to J_\bullet$, and attaching each ancestor subtree~$X$ of~$J_{\circ\bullet}$ to~$J_\circ$ if there is an ancestor subtree~$Y$ of~$J_{\circ\bullet}$ with~$X < Y < \min(J_\bullet)$ and to~$J_\bullet$ otherwise, and similarly each descendant subtree~$X$ of~$J_{\circ\bullet}$ to~$J_\bullet$ if there is a descendant subtree~$Y$ of~$J_{\circ\bullet}$ with~$\max(J_\circ) < Y < X$ and to~$J_\circ$ otherwise.
By definition, $T$ is a tree on the parts of a partition of~$[n]$, and $S$ is the contraction of the edge~$J_\circ \to J_\bullet$ in the tree~$T$, so we just need to check that~$T$ is a Schröder separating tree.
Consider two distinct ancestor subtrees~$X$ and~$Y$ of a node~$J$ of~$T$ (the case of descendant subtrees is symmetric).
if~$J \notin \{J_\circ, J_\bullet\}$, there is nothing to prove.
Assume first that~$J = J_\bullet$.
...
%\begin{itemize}
%\item if~$I = K$ or~$I \leftarrow K$ is an arc of~$S$, then there is a directed path in~$S$ joining either~$I$ to~$J$, or~$J$ to~$K$,
%\item if~$I = K$ or~$I \to K$ is an arc of~$S$, then there is a directed path in~$S$ joining either~$J$ to~$I$, or~$K$ to~$J$.
%\end{itemize}
\end{lemma}
...
\begin{proof}
Consider the contraction~$T$ of an edge~$J_\circ \to J_\bullet$ in a $\equiv$-admissible Schröder separating tree~$S$, and denote by~$J_{\circ\bullet} \eqdef J_\circ \sqcup J_\bullet$ the merged node.
...
Consider first an arc~$(i,k,A,B)$ of~$T$.
...
Consider now a node~$J$ of~$T$ with two distinct ancestor subtrees~$L$ and~$R$ with~$L < R$, and let~$M \eqdef \max(L)$ and~$m \eqdef \min(R)$.
If~$L$ and~$R$ are both ancestor subtrees of the same node of~$S$, then the up arc~$(M, m, {]M,m[}, \varnothing)$ is not in~$\arcs$ since~$S$ is $\equiv$-admissible.
Otherwise, we have~$J = J_{\circ\bullet}$, and $L$ is a descendant subtree of~$J_\circ$ while~$R$ is a descendant subtree of~$J_\bullet$ (or the opposite).
Let~$L'$ be the descendant subtree of~$J_\bullet$ in~$S$ containing~$L$, and let~$M' \eqdef \max(L')$.
As~${L \subseteq L'}$, we have $M < M'$, so that the arc~$(M', m, {]M',m[}, \varnothing)$ is a subarc of the arc~$(M, m, {]M,m[}, \varnothing)$.
We conclude that the arc~$(M, m, {]M,m[}, \varnothing)$ is not in~$\arcs$ since its subarc~$(M', m, {]M',m[}, \varnothing)$ is not in~$\arcs$ since~$S$ is $\equiv$-admissible.
The proof is symmetrical for a node~$J$ with two distinct descendant subtrees.
\end{proof}
...
Moreover, if~$I \ne I'$ or~$K \ne K'$, we have either~$I' = K' = I \cup K = J$ or~$I' = J \ne K = K'$ or~$I = I' \ne J = K'$.
In the first case, ${]i,k[} \cap (I' \cup K') = {]i,k[} \cap (I \cup K) = \varnothing$.
In the second case, since~$I$ and~$K$ form an edge of~$T$, we obtain by construction that~$I = J_\circ$ and~$\max(K) < \max(J_\circ) < \min(J_\bullet)$ or~$I = J_\bullet$ and~$\max(J_\circ) < \max(K)$, from which we deduce that~$I' \cap K' = J \cap K = I \cap K$, hence~${]i,k[} \cap (I' \cup K') = {]i,k[} \cap (I \cup K) = \varnothing$.
The third case is symmetrical.
We conclude that~$(i,k,A,B)$ is also an arc of~$S$, hence it belongs to~$\arcs$ since~$S$ is $\equiv$-separating.

Consider now a node~$J$ of~$T$ with two distinct ancestor subtrees~$L$ and~$R$ with~$L < R$, and let~$M \eqdef \max(L)$ and~$m \eqdef \min(R)$.
\vincent{todo: I have a problem here...}
\end{proof}
