...
Consider now a node~$J$ of~$R$ with two distinct ancestor subtrees~$L$ and~$R$ with~$L < R$, and let~$M \eqdef \max(L)$ and~$m \eqdef \min(R)$.
...
Otherwise, we have~${J = J_\circ \cup J_\bullet}$, and $L$ is a descendant subtree of~$J_\circ$ while~$R$ is a descendant subtree of~$J_\bullet$ (or the opposite).
