...
\vincent{should we use subarc order instead?}
...
A lattice congruence~$\equiv$ of the weak order is \defn{simple} if all maximal (in the arc poset) arcs of~$\arcs_n \ssm \arcs_\equiv$ are up arcs~$u(i,j)$ or down arcs~$u(i,j)$ with~$j-i > 1$.
\vincent{should we say subarc minimal instead?}
\vincent{We need to say somewhere that we only care about essential congruences. Hence we want strict up arcs and strict down arcs.}
\end{definition}
...
Note that the permutree congruences of~\cite{PilaudPons-permutrees} are by definition the simple congruences for which all maximal arcs of~$\arcs_n \ssm \arcs_\equiv$ have length~$2$, meaning that they are of the form~$(i-1, i+1, \{i\}, \varnothing)$ or~$(i-1, i+1, \varnothing, \{i\})$ for some~$2 \le i \le n-1$.
\vincent{should we say subarc minimal instead?}
\end{remark}
...
\begin{remark}
As already observed in~\cite[Sect.~4.4]{HoangMutze}, the number of simple congruences of the weak order on~$\f{S}_n$ is~$C_{n-1}^2$, where~$C_n \eqdef \frac{1}{n+1} \binom{2n}{n}$ is the Catalan number.
\vincent{Be careful that it only takes into account essential congruences here...}
\end{remark}
